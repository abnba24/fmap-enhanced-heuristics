...
public abstract class HeuristicFactory {
    ...
    public static final String[] HEURISTICS = {"FF", "DTG", "DTG + Landmarks", 
        "Inc. DTG + Landmarks", "Centroids", "MCS (Min. Covering States)"};
    ...
    public static final int CENTROIDS = 4;      // Centroids heuristic (minimizes mean cost to goals)
    public static final int MCS = 5;            // Minimum Covering States (minimizes max cost to goals)
        ...
        if (infoFlag == INFO_USES_LANDMARKS) {
            switch (heuristic) {
                ...
                case LAND_DTG_INC:
                    ...
                    break;
                case CENTROIDS:
                    res = "no";  // Can optionally use landmarks through inner heuristic, but not inherently
                    break;
                case MCS:
                    res = "no";  // Can optionally use landmarks through inner heuristic, but not inherently
                    break;
            ...
            switch (heuristic) {
                ...
                case LAND_DTG_INC:
                    ...
                    break;
                case CENTROIDS:
                    res = "yes";  // Distributed heuristic that aggregates costs across agents
                    break;
                case MCS:
                    res = "yes";  // Distributed heuristic that aggregates costs across agents
                    break;
